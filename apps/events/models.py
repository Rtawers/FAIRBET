...
class Market(models.Model):
    ...
    margin = models.DecimalField(max_digits=5, decimal_places=2, default=Decimal("5.00"))
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class Selection(models.Model):
    ...
    result  = models.CharField(max_length=10, choices=SelectionResult.choices, default=SelectionResult.PENDING)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def clean(self):
        if self.odds is not None and self.odds <= Decimal("1.00"):
            raise ValidationError({"odds": f"Las odds deben ser > 1.00. Recibido: {self.odds}"})
