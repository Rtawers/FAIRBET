...
def place_bet_view(request):
    place_bet_view.throttle_scope = "bet"
...
@api_view(["POST"])
@permission_classes([IsAuthenticated])  # Cualquier usuario autenticado puede liquidar
def settle_bet_view(request):
    ...
    try:
        ...
        # Solo el dueño de la apuesta puede liquidarla
        if bet.user != request.user:
            return Response({"error": "No es tu apuesta"}, status=status.HTTP_403_FORBIDDEN)
